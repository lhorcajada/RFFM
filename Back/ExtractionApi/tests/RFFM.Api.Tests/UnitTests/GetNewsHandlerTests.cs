...
namespace RFFM.Api.Tests.UnitTests
{
    ...
    public class GetNewsHandlerTests
    {
        ...
        public async Task Handle_WithLinkTypeNone_ReturnsNullLinkFields()
        {
            ...
            Assert.Equal("None", result[0].LinkType);
        ...
        public async Task Handle_ReturnsBody()
        {
            ...
            await using var seedDb = _fixture.CreateDbContext();
            var news = NewsItem.Create(
                "Title", "Sub", "Cuerpo completo de la noticia.", "https://example.com/image.jpg", NewsStatus.Published,
                new DateTime(2026, 9, 1, 0, 0, 0, DateTimeKind.Utc)
            );
            seedDb.News.Add(news);
            await seedDb.SaveChangesAsync();
            ...
            Assert.Single(result);
            Assert.Equal("Cuerpo completo de la noticia.", result[0].Body);
        }
    }
}
