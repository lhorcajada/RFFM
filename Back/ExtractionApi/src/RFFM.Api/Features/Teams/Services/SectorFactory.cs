namespace RFFM.Api.Features.Teams.Services
{
    ...
    {
        public List<RFFM.Api.Features.Teams.Queries.Responses.Sector> BuildSectors(int matchTime, int sectorsPerHalf)
        {
            var halfDuration = matchTime / 2;
            var sectors = new List<RFFM.Api.Features.Teams.Queries.Responses.Sector>();
            ...
            for (var half = 0; half < 2; half++)
            {
                ...
                for (var s = 0; s < sectorsPerHalf; s++)
                {
                    ...
                    sectors.Add(new RFFM.Api.Features.Teams.Queries.Responses.Sector { StartMinute = start, EndMinute = end, GoalsFor = 0, GoalsAgainst = 0 });
                    currentStart = end + 1;
                }
            }
