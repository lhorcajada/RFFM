...
namespace RFFM.Api.Features.Teams.Queries
{
    public class GetParticipationSummary : IFeatureModule
    {
        ...
        public class ParticipationRequestHandler : IRequestHandler<ParticipationQuery, ParticipationCount[]>
        {
            private readonly ITeamService _teamService;
            private readonly IPlayerService _playerService;

            public ParticipationRequestHandler(ITeamService teamService, IPlayerService playerService)
            {
            ...
            public async ValueTask<ParticipationCount[]> Handle(ParticipationQuery request, CancellationToken cancellationToken)
            {
                ...
                if (team == null || team.Players == null || !team.Players.Any())
                    return Array.Empty<ParticipationCount>();
                ...
                var tasks = team.Players.Select(async p =>
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(playerId))
                    {
                        try
                        {
                            pd = await _playerService.GetPlayerAsync(playerId!, request.SeasonId, cancellationToken);
                        }
                        catch
                        {
