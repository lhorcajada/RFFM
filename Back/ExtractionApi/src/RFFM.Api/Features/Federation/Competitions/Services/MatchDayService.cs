...
namespace RFFM.Api.Features.Federation.Competitions.Services
{
    ...
    public class MatchDayService : IMatchDayService
    {
        ...
        public async Task<List<MatchDayResponse>> GetMatchDays(int groupId, CancellationToken cancellationToken)
        {
            ...
            var groupRoundsRffm = JsonSerializer.Deserialize<GroupRoundRffm>(content, options);
            if(groupRoundsRffm == null)
                return [];
            return groupRoundsRffm.MatchDays.Select(m => new MatchDayResponse
            {
