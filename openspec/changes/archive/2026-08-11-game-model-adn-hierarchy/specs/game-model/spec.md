...
Removed — `GameScenario` no longer exists. There is no scenario-move endpoint in the new hierarchy.

### Requirement: Scenarios are grouped under a Principio within each moment/zone cell
Removed — superseded by the ADN hierarchy (`Fase → Principio → Subprincipio → Zona → SubSubPrincipio → Habilidad`). `GamePrinciple` no longer belongs to a Momento×Zona cell; it belongs to a Fase only, and Zona now hangs off `Subprincipio`.

### Requirement: Existing scenarios are migrated into one generated principle each
Removed — this change is a full, disposable replacement of existing game-model data; there is no scenario-to-principle migration path.
