...
de entrenamiento presente en la sesión (Táctico > Técnico > Físico = 0), y cada partido pondera
sus minutos por tipo de partido (Liga > Amistoso/Torneo).
...
#### Scenario: Un entreno puramente Físico no aporta nada a Rodaje
- **WHEN** un jugador asiste a una sesión de entrenamiento marcada únicamente como `Fisico`
- **THEN** esa sesión contribuye `0` al `TrainingComponent`, independientemente de su resultado
  de asistencia
...
- **THEN** la contribución de esa sesión al `TrainingComponent` es la media de los pesos de
  `Tactico` y `Fisico` (no cero, y no el peso pleno de `Tactico`), sin verse afectada por el peso
  de `Tecnico` (ausente en esa sesión)
