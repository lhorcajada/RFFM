  ...
  const groups: { title: string; tiles: { key: string; visible: boolean; node: JSX.Element }[] }[] = [
    ...
    },
    {
      ...
      tiles: [
        ...
        {
          key: "notifications",
          // No feature-permission gate: every authenticated role (Coach/Player/FamilyMember)
          // has their own notification inbox, scoped server-side to their own user id.
          visible: true,
          node: (
            <LauncherTile
              title="Notificaciones"
              illustration={<NotificationsIllustration />}
              gradient="linear-gradient(135deg, #4a148c 0%, #6a1b9a 50%, #38006b 100%)"
